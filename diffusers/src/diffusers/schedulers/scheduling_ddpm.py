...
class DDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_to_init(
        ...
        """
        Predict the sample when t = 0 by reversing the SDE. Core function to propagate the diffusion
        process from the learned model outputs (most often the predicted noise).
        ...
        # prev_t = self.previous_timestep(t)
        prev_t = torch.LongTensor([0] * timestep.shape[0])
        ...
        # 1. compute alphas, betas
        alpha_prod_t = self.alphas_cumprod[t.cpu()].to(device)
