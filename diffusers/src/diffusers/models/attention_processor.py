...
class Attention_New(nn.Module):
    ...
    def __init__(
        ...
        if processor is None:
            processor = (
                AttnProcessor_New2_0() if hasattr(F, "scaled_dot_product_attention") and self.scale_qk else AttnProcessor()
            )
        self.set_processor(processor)
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, other_info={}, **cross_attention_kwargs):
        # The `Attention` class can call different attention processors / attention functions
        # here we simply pass along all tensors to the selected processor class
        # For standard processors that are defined here, `**cross_attention_kwargs` is empty
        return self.processor(
...
class AttnProcessor_New2_0:
    ...
    def __call__(
        ...
        hola_loss = {}
        if 'loss_function' in other_info:
            batch = other_info['batch']
            attention_maps = torch.softmax((query @ key.transpose(-2, -1) / math.sqrt(query.size(-1))), dim=-1)

            if attention_maps.shape[3] != 77:
                pass 
            else:
                for loss_f_name in other_info['loss_function']:
                    loss_res, attn_map_scale = other_info['loss_function'][loss_f_name](attention_maps, batch)
                    """
                    loss_res: [
                        batch1: [res1, res2],
                        batch2: [res1, res2],
                    ...
                    if loss_res is not None:
                        hola_loss[loss_f_name + '_{}'.format(attn_map_scale)] = loss_res
        ...
        """
        torch.softmax((query @ key.transpose(-2, -1) / math.sqrt(query.size(-1))), dim=-1)
        """
